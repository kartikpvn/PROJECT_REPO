    ...
    , f_addr(addr)
{
    ...
    //std::string decimal_port = std::to_string(port);
...
{
   return  recvfrom(f_socket, msg, max_size,0,(struct sockaddr *)&srcaddr, &srcaddrlen);
}
...
{
    return ::sendto(f_socket, msg, size,0,(struct sockaddr *)&destaddr, destaddrlen);
 }
