...
sched_cancel(struct kthread *kthr)
{
        if(kthr->kt_state == KT_SLEEP_CANCELLABLE) {
            /*dbg(DBG_PRINT,"(GRADING1C)\n");*/
               ...
               /* sched_switch();*//*why sched_switch here*/
        } else {
               /* dbg(DBG_PRINT,"(GRADING1C)\n");*/
                kthr->kt_cancelled = 1;
        }
 ...
 * your run queue will remain empty. Thisn is very subtle, but
...
sched_switch(void)
{
        /* Impemented as per code given in lecture slides 73*/
        dbg(DBG_PRINT,"(GRADING1A)\n");
        kthread_t *oldThread;
    /*dbg(DBG_PRINT,"entered sched_switch and current runq : %d\n",kt_runq.tq_size);*/
        int oldIPL = intr_getipl();
        intr_setipl(IPL_HIGH);

        /* protect access to RunQueue by masking all interrupts*/
        kthread_t *loopthread = ktqueue_dequeue(&kt_runq);
        while(loopthread==NULL)
        {
            dbg(DBG_PRINT,"(GRADING1C)\n");
                ...
                intr_wait();
                intr_setipl(IPL_HIGH); 
                loopthread=ktqueue_dequeue(&kt_runq);
        }
        ...
        context_switch(&oldThread->kt_ctx,&loopthread->kt_ctx);
        intr_setipl(oldIPL);
 /*dbg(DBG_PRINT,"exit sched_switch\n");*/
