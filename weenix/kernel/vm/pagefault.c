...
{
    dbg(DBG_TEST,"enter handle_pagefault\n");
	/*vmarea that contains the address that was faulted on*/
    vmarea_t *vmarea;
    pframe_t *p;
    int forwrite, errno;
    uint32_t pagenum, pdflags, ptflags;

    vmarea = vmmap_lookup(curproc->p_vmmap, ADDR_TO_PN(vaddr));
    if (vmarea == NULL) {
    	do_exit(EFAULT);
        /*do_waitpid(curproc->p_pid, 0, EFAULT);*/
    }
    ...
    if (prot & PROT_NONE) {
        do_exit(EFAULT);
        /*do_waitpid(curproc->p_pid, 0, EFAULT);*/
    } else if ((cause & FAULT_WRITE) && !(prot & PROT_WRITE)) {
    	do_exit(EFAULT);
        /*do_waitpid(curproc->p_pid, 0, EFAULT);*/
    } else if ((cause & FAULT_EXEC) && !(prot & PROT_EXEC)) {
    	do_exit(EFAULT);
        /*do_waitpid(curproc->p_pid, 0, EFAULT);*/
    ...
    pagenum = ADDR_TO_PN(vaddr) - vmarea->vma_start + vmarea->vma_off;
    if(cause & FAULT_WRITE) {
    	forwrite = 1;
    } else {
    	forwrite = 0;
    }
    errno = pframe_lookup(vmarea->vma_obj, pagenum, forwrite, &p);   
    if (errno < 0){
        do_exit(EFAULT);
        /*do_waitpid(curproc->p_pid, 0, EFAULT);*/
    }
    ...
    if(cause & FAULT_WRITE) {
    	pdflags = PD_PRESENT | PD_WRITE | PD_USER;
    	ptflags = PT_PRESENT | PT_WRITE | PT_USER;	
    } else {
    ...
    dbg(DBG_TEST,"exit handle_pagefault\n");
}
