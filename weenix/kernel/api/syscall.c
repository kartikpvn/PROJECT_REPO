...
{
    dbg(DBG_TEST,"enter sys_read\n");
    read_args_t rargs;
    int errno;
    int read = 0;

    /*copy_from_user() the write_args_t*/
    errno = copy_from_user(&rargs, arg, sizeof(read_args_t));
    if (errno < 0){
    ...
    /*call do_read(), and copy_to_user() the write bytes*/
    read = do_read(rargs.fd, buf, rargs.nbytes);
    if(read < 0) {
        page_free(buf);
    ...
    errno = copy_to_user(arg->buf, buf, read);
    if (errno < 0) {
        page_free(buf);
        curthr->kt_errno = -errno;
        return -1;
...
{
    dbg(DBG_TEST,"enter sys_write\n");
    write_args_t wargs;
    int errno;
    int written = 0;

    /*copy_from_user() the write_args_t*/
    errno = copy_from_user(&wargs, arg, sizeof(write_args_t));
    if (errno < 0){
        curthr->kt_errno = -errno;
        return -1;
    }
    ...
    errno = copy_from_user(buf, (void*)wargs.buf, wargs.nbytes);
    /*call do_write(), and copy_to_user() the write bytes*/
    written = do_write(wargs.fd, buf, wargs.nbytes);
    if(written < 0) {
        page_free(buf);
        curthr->kt_errno = -written;
        return -1;
    }

/*errno = copy_to_user(wargs.buf, buf, written);
    if (errno < 0) {
        page_free(buf);
        curthr->kt_errno = -errno;
        return -1;
