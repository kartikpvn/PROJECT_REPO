...
addr_perm(struct proc *p, const void *vaddr, int perm)
{
    ...
    vmmap = p->p_vmmap;
    vmareaAddr = ADDR_TO_PN(vaddr);
    vmarea = vmmap_lookup(vmmap, vmareaAddr);

    /*if(vmarea != NULL) {
        protection = vmarea->vma_prot;
        if((protection & PROT_READ) == (perm & PROT_READ) && (protection & PROT_WRITE) == (perm & PROT_WRITE) && (protection & PROT_EXEC) == (perm & PROT_EXEC)) {
            return 1;
        }
    } */
...
{
    dbg(DBG_TEST,"enter range_perm\n");
    uint32_t vaddr, eaddr;
    vaddr = (uint32_t) avaddr;
    eaddr = (uint32_t) avaddr + len;

    while(eaddr > vaddr) {
        if(addr_perm(p, (void *)vaddr, perm)!= NULL) {
            vaddr += PAGE_SIZE;
        } else {
            dbg(DBG_TEST,"exit range_perm\n");
            return 0;
        }
    }
    dbg(DBG_TEST,"exit range_perm\n");
